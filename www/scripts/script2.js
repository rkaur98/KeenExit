...
function result(status) {
    ...
    firebase.auth().onAuthStateChanged(function (user) {
        if (user) {
            ...
            if (user != null) {
                firedata.ref('users/' + user.uid + '/Level2/Time').set(completeTime);
                firedata.ref('users/' + user.uid + '/Stage').set('3');
                ...
                if (status == "complete") {
                    ...
                    buttons.innerHTML = buttons.innerHTML + '<a href="level3.html" class="grnbtn">Next Level</a>';
                }
                else {
                    res.innerHTML = "<img src='images/tryagain.png' />";
                    buttons.innerHTML = buttons.innerHTML + '<a href="level2.html" class="grnbtn">Try Again</a>';
                }
            }
